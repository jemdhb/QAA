...
def get_counts(fh):
    ...
    ax.set(title=f"{args.filename[:args.filename.index(".")]}: Number of genes with certain counts",ylabel="Number of genes",
           xlabel="Counts")
...
def get_categories(fh):
    ...
    #for our summary categories, create a list of tuples formatted (category, count)
    cats=[item.split()[0].replace("_"," ").strip() for item in data if item.split("\t")[0].startswith("__")]
    ...
    sns.despine(top=True,right=True)
    ax.set(title=f"{args.filename[:args.filename.index(".")]}: Number of reads per category",ylabel="Counts",
           xlabel="Category")
